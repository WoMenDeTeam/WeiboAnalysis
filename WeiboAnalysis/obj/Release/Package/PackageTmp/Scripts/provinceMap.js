...
(function (exports) {
    ...
    var ProvinceView = exports.ProvinceView = shu.ChartView.extend(function (container) {
        ...
        // china={{name, path, data={name, value, ratio}}}
        var china = {};
        //循环切片方式绘制地图 - IE7下有所提升, IE6依然会卡
        function slicedDraw(arr, func, options) {
            var settings = {
            ...
            function wrap() {
                ...
                setTimeout(wrap, settings.wait);
            }
            wrap();
        }
        slicedDraw(null, function (p, index) {
            china[p] = {
                name: p,
                path: R.path(CHINA_PATH[p]).attr(defaultAttr)
            };
            $(china[p].path.node).data('name', p);
            self.renderArea(china[p], index);
        }, {
            completed: function () {
                var k = 0, set = R.set();
                // R.transform('translate( -225*(0.7-1), -180*(0.7-1) ) scale(0.7)'); // 调整地图大小及位置
            }
        });

        //绘制左上的颜色图例
        var lastX = 15;
        this.gradColor(settings.maxColor, settings.minColor, settings.colorNum, function (index, color) {
            lastX += 11;
            //图例小块的大小和位置
            var rect = R.rect(lastX, 430, 10, 10).attr({
                fill: 'rgb(' + color.r + ',' + color.g + ',' + color.b + ')',
                stroke: 'rgb(' + color.r + ',' + color.g + ',' + color.b + ')'
            });
        });
        R.text(10, 435, '高').attr(defaultFont);
        R.text(10 + 11 * settings.colorNum + 15, 435, '低').attr(defaultFont);
        R.tmpPath = null;
        //初始化hover的tooltip框
        R.tooltext = R.text(480, 200).attr({
            opacity: 0,
            'font-size': 18,
            'font-weight': 'bold',
            'fill': '#fff'
        ...
        var t = this.settings.rankRect;
        //右侧排名
        //R.text(t.x, t.y, "省份热度排名").attr(defaultFont);
        /*
        this.ranklist= [];
        for (var i=0; i<10; i++) {
        t= document.createElementNS('tspan');
        t.setAttributeNS(null, "x", "0");
        t.setAttributeNS(null, "dy", "1em");
        rlist.node.appendChild(t);
        this.ranklist.push(t);
        }*/
        /*同时注释掉 334:修正城市排名
        R.ranklist = [];
        var y = t.y;
        for (var i = 0; i < 10; i++) {
        y += t['line-height'];
        R.ranklist.push(R.text(t.x, y, " ").attr(defaultFont));
        };
        */
        this.R = R;
        this.china = china;
    ...
    ProvinceView.prototype.renderArea = function (area, index) {

        //将地图按比例缩放
        /*
        area.path.scale(0.7, 0.7).translate(-bbox.x * 0.3, -bbox.y * 0.3);
        var bbox = area.path.getBBox();
        area.path.scalePos = {
        x: 220 - (bbox.x + bbox.width / 2),
        y: 240 - (bbox.y + bbox.height / 2),
        w: bbox.width,
        h: bbox.height
        };
        */
        //缩放 by rensq
        area.path.scale(0.8, 0.8, 0, 100)
                        .translate(0, 5)
                        .attr({
                            //                    title:area.name
                        });

        var min = 155 + index % 3 * 20, s = 255 - min;
        var c1 = Math.random() * s + min
        , c2 = Math.random() * s + min
        , c3 = Math.random() * s + min;
        var randomHoverColor = 'rgba(' + c1 + ',' + c2 + ',' + c3 + ',' + 0.8 + ')';

        var self = this;
        $(area.path.node).css('cursor', 'pointer');
        area.path.hover(function () {
            area.lastColor = this.attr('fill');
            this.attr({
                fill: self.settings.hoverColor
            });
        }, function () {
            this.attr({
                fill: area.lastColor
    ...
    /****
    * 渐变色生成
    * @param callback: function(index, color)
    */
    ProvinceView.prototype.gradColor = function (first, last, k, callback) {
        var my_colors = new Array("#eb143f", "#d9712b", "#f2f310", "#15e9a2", "#99cccc");
        var c = {};
        for (var i = 0; i < k; i++) {
            var cur_color = {}
            cur_color = Raphael.getRGB(my_colors[i]);
            callback(i, cur_color);
        }
    ...
    * each 地图中的省份
    */
    ProvinceView.prototype.eachPart = function (callback) {
        var china = this.china
        for (p in china) {
            callback.call(this, china[p]);
        }
    };
    ...
    * 读入新数据, 并由此更新地图着色等
    */
    ProvinceView.prototype.render = function render(data, options) {
        var self = this,
            settings = this.settings;
        if (!window.map_options) window.map_options = options;
        this.data = data;
        if (this.isRedrawing) {     //如果在重绘地图则等待其完成
            setTimeout(function () {
                self.render(data);
            }, 10);
            return;
        ...
        for (var i = 0, len = data.length; i < len; i++) {
            ...
            if (i > 0 && d.value == data[i - 1].value) {
                d.rank = data[i - 1].rank;
            } else {
                d.rank = i + 1;
            }
            $(p.path.node).data('data', d); //$缓存data以便hover时使用
            //调整tooltip显示数据
            /*绑定鼠标单击事件*/
            //window.map_options.callback==""
            if (typeof window.map_options.clickfun === "function") {
                p.path.unclick().click(window.map_options.clickfun);
            }

            /*绑定鼠标悬停事件*/
            p.path.unhover().hover(function (e) {
                var d = $(this.node).data('data');
                if (!d) {
                    return;
                }
                var tips_name = (window.map_options && window.map_options.tips_name) ? window.map_options.tips_name : '舆情热度';
                d = ([d.name, tips_name + "：" + d.value, "占比(全国)：" + d.rate.toString().slice(0, 3) + "%", "排名(全国)：" + d.rank].join('<br/>'));

                e = jQuery.event.fix(e);
                if (d.value != "0") {
                    tooltip.render(d).show(e.pageX, e.pageY);
                }
        ...
        //var step = Math.floor(lenToPaint / settings.colorNum) || 1;    //计算每种颜色对应的省份数量
        var step = 5;
        this.gradColor(settings.maxColor, settings.minColor, settings.colorNum, function (index, color) {
            for (var i = 0; i < step; i++) {
                var cur_color = color;
                var d = data[index * step + i];
                if (!d || d.value === minValue) {
                    break;
                } else {
                    var pre_d = data[index * step + i - 1];
                    if (pre_d != null && d.rank == pre_d.rank) {
                        cur_color = pre_d.color;
                    }
                    d.color = cur_color;
                    var c = (['rgb(', cur_color.r, ',', cur_color.g, ',', cur_color.b, ')'].join(''));
                    var p = china[d.name];
                    if (p) {
                        p.path.attr({
                            fill: c
        ...
        var str = (i + 1) + " " + data[i].name + ' ' + data[i].rate + '%';
        this.R.ranklist[i].attr({ text: str.slice(0, 9) + '%', 'font-family': 'SimSun', 'font-size': '12px' });
        }
        */
    };

    var NewProvinceView = exports.NewProvinceView = shu.ChartView.extend(function (container) {
        this.R = container;
    });
    NewProvinceView.prototype.render = function (data) {
        this.data = data;
        this.sorted = data.sort(function (a, b) {
            return b.value - a.value;
        });
        this.sliced = this.sorted.slice(0, 5);
        var template = $('<div><div class="title">热门省份Top5：</div><ul></ul></div>');
        var ul = template.find("ul");
        for (var i = 0, l = this.sliced.length; i < l; i++) {
            var rate = Math.min(this.sliced[i].rate * 4, 50) + this.sliced[i].rate / 10;
            this.sliced[i].width = rate;
            ul.append('<li class="item_' + i + '"><span>' + (i + 1) + ' ' + this.sliced[i].name + '</span><div class="rate"><div style="width: ' + rate + '%"></div> &nbsp; ' + this.sliced[i].rate.toFixed(1) + '%</div></li>');
        }
        this.R.html(template);
    };
})(function () {
    if (!window.shu) {
        window.shu = {};
