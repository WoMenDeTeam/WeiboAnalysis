// JavaScript Document
function TransRoute(obj, data, option,linecolor) {
    this.canvas_obj = obj;
    ...
    this.item_style = option == null ? {"border":"1px solid black","background":"none"} : option;
}
...
var whole_width = parseInt(this.canvas_width - this.display_num * this.item_width);
    ...
    for (var i = 0, j = this.trans_data.length; i < j; i++) {
        ...
        var index = 1;
        var compare_y = parseInt((this.canvas_height - 20) / site_len);
        var l_middle_y = parseInt(compare_y / 2);
        var l_x = i * (this.item_width + this.space_width);
        for (var k = 0; k < site_len; k++) {
            var l_y = k * compare_y + l_middle_y - parseInt(this.item_height / 2);
            if (index == 1) {
