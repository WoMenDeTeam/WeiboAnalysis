...
var Config = {
    ...
    HotWords: "奶粉,爆炸,火灾",
    //过滤关键词组
    FilterKeyWords: "恒天然,奶粉",
    //IDOL服务器与WEB应用服务器的时间差，以天为单位
    TimeSpan: 0,
    //微博所对应的IDOL库名
    weibodatabase: "NewPortal",
    //微博查询数据是需要返回的字段，默认为全部
    weiboprintfields: "all",
    //IDOL对应的所有库名
    AllDataBase: "NewPortal+NewBBS",
    //专题追踪的父级栏目ID
    ThemeParentCate: 202,
    protalWeb: "NewPortal",
    //微博所对应的IDOL库名
    weibodatabase: "NewPortal",
    MinZhengNews: "NewPortal",
    VideoHost: "http://123.103.15.153:8388/",
    LeaderVideoKeyWork: "\"付建华\",\"黄毅\",\"黄玉治\",\"李万疆\",\"彭建勋\",\"孙华山\",\"王德学\",\"王树鹤\",\"杨栋梁\",\"杨元元\",\"赵铁锤\",\"赵惠令\"",
    WeiboDatabase: "WeiboMilk",
    ...
    */
    DisplayHtml: function (data, obj, tag) {
        ...
        for (var item in data) {
            ...
            htmlcontent.push("<div class=\"new201252\">");
            htmlcontent.push("<b class=\"date new_par15\">" + unescape(entity["time"]).slice(0, -3) + "</b><span class=\"new_par15\">【" + unescape(entity["site"]) + "】</span>");
            //READNUM,FORWARDNUM,REPLYNUM,SAMENUM 阅读  ，转发数 ，回复数， 相同新闻数（媒）
            var ordertab = $("#orderTabUl").find("li[name='sort_search_type']");
            ordertab.each(function () {

                $(this).hide();
            });
            $("#orderTabUl").find("li[name='sort_search_type'][pid='Date']").show();


            switch (obj.dissamenews) {
                case "MATCH{news}:C1": //新闻
                    $("#orderTabUl").find("li[name='sort_search_type'][pid='READNUM:numberdecreasing']").css("display", "block");
                    $("#orderTabUl").find("li[name='sort_search_type'][pid='REPLYNUM:numberdecreasing']").css("display", "block");
                    $("#orderTabUl").find("li[name='sort_search_type'][pid='SAMENUM:numberdecreasing']").css("display", "block");
                    if (entity["readnum"]) {
                        htmlcontent.push("阅读量（<span class=\"new_red\">" + entity["readnum"] + "</span>）");
                    }

                    if (entity["replynum"]) {
                        htmlcontent.push("　评论（<a href=\"#\" target=\"_blank\">" + entity["replynum"] + "</a>)");

                    }
                    if (unescape(entity["samenum"]) > 0) {
                        htmlcontent.push("　相同新闻（<a name=\"search_samelinksuggest_result_" + unescape(entity["docid"]) + "\"   >" + entity["samenum"] + "</a>)");
                ...
                case "MATCH{bbs}:C1": //论坛
                    ...
                    if (entity["replynum"]) {
                        htmlcontent.push("　评论量（<a href=\"#\" target=\"_blank\">" + entity["replynum"] + "</a>)");

                    }

                    break;
                case "MATCH{blog}:C1": //博客
                    $("#orderTabUl").find("li[name='sort_search_type'][pid='REPLYNUM:numberdecreasing']").css("display", "block");
                    $("#orderTabUl").find("li[name='sort_search_type'][pid='READNUM:numberdecreasing']").css("display", "block");
                    if (entity["readnum"]) {
                        htmlcontent.push("阅读量（<span class=\"new_red\">" + entity["readnum"] + "</span>）");
                    ...
                    if (entity["author"]) {
                        htmlcontent.push("　发布人昵称（<a href=\"" + (entity["authorurl"] ? entity["authorurl"] : "#") + "\" target=\"_blank\">" + entity["author"] + "</a>)");
                    }
                    if (entity["replynum"]) {
                        htmlcontent.push("　评论量（<a href=\"#\" target=\"_blank\">" + entity["replynum"] + "</a>)");
                    }
                ...
                case "": //微博
                    $("#orderTabUl").find("li[name='sort_search_type'][pid='READNUM:numberdecreasing']").css("display", "block");
                    $("#orderTabUl").find("li[name='sort_search_type'][pid='FORWARDNUM:numberdecreasing']").css("display", "block");
                    $("#orderTabUl").find("li[name='sort_search_type'][pid='REPLYNUM:numberdecreasing']").css("display", "block");
                    if (entity["readnum"]) {
                        htmlcontent.push("阅读量（<span class=\"new_red\">" + entity["readnum"] + "</span>）");
                    }
                    if (entity["forwardnum"]) {
                        htmlcontent.push("　转发量（<a href=\"#\" target=\"_blank\">" + entity["forwardnum"] + "</a>)");
                    }
                    if (entity["author"]) {
                        htmlcontent.push("　发布人昵称（<a href=\"" + (entity["authorurl"] ? entity["authorurl"] : "#") + "\" target=\"_blank\">" + entity["author"] + "</a>)");
                    }
            ...
            if (obj.dissamenews != "MATCH{news}:C1") {
                //if (unescape(entity["samenum"]) > 0) {
                //相同新闻
                //  htmlcontent.push("<a name=\"search_samelinksuggest_result_" + unescape(entity["docid"]) + "\"  style=\"display:none;\"  class=\"link_off2\" href=\"javascript:void(null);\"></a>");
                //  htmlcontent.push("<ul style=\"display:none;\" class=\"news_list\" samenum=\"" + unescape(entity["samenum"]) + "\" hiddentxt=\"" + entity["href"] + "\" id=\"samelinksuggest_" + unescape(entity["docid"]) + "\"></ul>");
                ...
                if (data) {
                    for (var item in data) {
                        ...
                        if (con.indexOf("<a href=") == -1) {
                            ...
                            $("[name='search_samelinksuggest_result_" + item + "']").text($(con + "li").length); //"共有" + $(con + "li").length + "条相同新闻");
